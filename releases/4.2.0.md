# Stencyl 4.2.0 — Development Release
...
Notable changes include support for Apple Silicon without Rosetta, better UI on Linux, dark theme, more detailed build progress, and changes to extensions development and distribution.

Please try it out by downloading from the below link, and let us know on Discord or on the forum if you have feedback about the new features or if you run into any problems with this release.

## Download

[Multiplatform Release](https://www.stencyl.com/download/get/4.2.0/multiplatform/) (Windows, macOS, Linux)
...
- [windows] Fix Windows launcher error when JDK is not yet installed and a proxy or firewall causes ssl revocation server to be unreachable
...
- Show both sha1 and sha256 fingerprints for android certificates
- Block old version of `androidx.fragment` which has been marked as outdated
...
- [all] Use binary format for bitmap fonts for a small boost to startup time for games with a large number of fonts
- [all] Log error for null or unreadable image in Image API functions
- [all] Fix incorrect graphics clipping at high resolutions when using fullscreen shaders

**Extensions**
- [general] [Stencyl Extension Gradle Plugin](https://github.com/Stencyl/stencyl-extension-gradle-plugin)
- [general] Updated [Toolset Extension Sample](https://github.com/Stencyl/toolset-extension-sample)
- [general] Extension metadata is now found in `stencyl-extension.json`, with older `info.txt` and `.jar` manifests still supported for backward compatibility.
- [general] Toolset extensions can be distributed as source code, with Stencyl compiling them on the spot based on the sources and dependencies defined in the `stencyl-extension.json`.
